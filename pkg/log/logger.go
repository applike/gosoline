...
type LoggerSettings struct {
	Handlers []HandlerSettings `cfg:"handlers"`
}
...
func (l *gosoLogger) Option(options ...Option) error {
	for _, opt := range options {
		if err := opt(l); err != nil {
			return fmt.Errorf("can not apply option %T", err)
		}
	}
...
func (l *gosoLogger) Error(format string, args ...interface{}) {
	...
	l.log(PriorityError, format, args, err)
}
...
func (l *gosoLogger) log(level int, msg string, args []interface{}, err error) {
	...
	for _, handler := range l.handlers {
		...
		if err := handler.Log(timestamp, level, msg, args, err, l.data); err != nil {
			l.err(err)
		}
	}
}
